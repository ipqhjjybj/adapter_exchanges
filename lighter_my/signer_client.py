...
def process_api_key_and_nonce(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            created_tx, ret, err = func(self, **partial_arguments, nonce=nonce, api_key_index=api_key_index)
...
class SignerClient:
    ...
    def change_api_key(self, eth_private_key: str, new_pubkey: str, nonce=-1):
        ...
        api_response = self.send_tx(tx_type=tx_type, tx_info=tx_info)
    ...
    def create_market_order_limited_slippage(
            self,
            market_index,
            client_order_index,
            base_amount,
        ...
        if ideal_price is None:
            order_book_orders = self.order_api.order_book_orders(market_index, 1)
            logging.debug(
    ...
    def create_market_order_if_slippage(
            self,
            market_index,
            client_order_index,
            base_amount,
        ...
        order_book_orders = self.order_api.order_book_orders(market_index, 100)
        if ideal_price is None:
            ideal_price = int((order_book_orders.bids[0].price if is_ask else order_book_orders.asks[0].price).replace(".", ""))
    ...
    @process_api_key_and_nonce
    def cancel_order(self, market_index, order_index, nonce=-1, api_key_index=-1) -> (CancelOrder, TxHash, str):
    ...
    def create_tp_order(self, market_index, client_order_index, base_amount, trigger_price, price, is_ask, reduce_only=False, nonce=-1,
    ...
    def create_tp_limit_order(self, market_index, client_order_index, base_amount, trigger_price, price, is_ask, reduce_only=False, nonce=-1,
                                    api_key_index=-1) -> (CreateOrder, TxHash, str):
        return self.create_order(
            market_index,
            client_order_index,
    ...
    def create_sl_order(self, market_index, client_order_index, base_amount, trigger_price, price, is_ask, reduce_only=False, nonce=-1,
                              api_key_index=-1) -> (CreateOrder, TxHash, str):
        return self.create_order(
            market_index,
            client_order_index,
    ...
    def create_sl_limit_order(self, market_index, client_order_index, base_amount, trigger_price, price, is_ask, reduce_only=False, nonce=-1,
                                    api_key_index=-1) -> (CreateOrder, TxHash, str):
        return self.create_order(
            market_index,
            client_order_index,
    ...
    @process_api_key_and_nonce
    def withdraw(self, usdc_amount, nonce=-1, api_key_index=-1) -> (Withdraw, TxHash):
    ...
    def create_sub_account(self, nonce=-1):
    ...
    @process_api_key_and_nonce
    def cancel_all_orders(self, time_in_force, timestamp_ms, nonce=-1, api_key_index=-1):
    ...
    @process_api_key_and_nonce
    def transfer(self, eth_private_key: str, to_account_index, usdc_amount, fee, memo, nonce=-1, api_key_index=-1):
        usdc_amount = int(usdc_amount * self.USDC_TICKER_SCALE)
    ...
    @process_api_key_and_nonce
    def mint_shares(self, public_pool_index, share_amount, nonce=-1, api_key_index=-1):
    ...
    @process_api_key_and_nonce
    def burn_shares(self, public_pool_index, share_amount, nonce=-1, api_key_index=-1):
    ...
    @process_api_key_and_nonce
    def update_leverage(self, market_index, margin_mode, leverage, nonce=-1, api_key_index=-1):
    ...
    @process_api_key_and_nonce
    def update_margin(self, market_index: int, usdc_amount: float, direction: int, nonce: int = -1):
        usdc_amount = int(usdc_amount * self.USDC_TICKER_SCALE)
    ...
    def send_tx(self, tx_type: StrictInt, tx_info: str) -> RespSendTx:
        if tx_info[0] != "{":
            raise Exception(tx_info)
        return self.tx_api.send_tx(tx_type=tx_type, tx_info=tx_info)
